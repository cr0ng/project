...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('.mdbtn').click(function(){
			...
			var tbody = $(this).parent().siblings().eq(2).text();
			$('#review_num').val(tno);
			$('#review_text').val(tbody);		
		});
...
<body>
	...
	<form method="POST" action="/project/movie/movieList.proj" id="frm" name="frm">
		...
		<input type="hidden" name="pno" id="pno" value=0> <!-- 글번호 전송용태그 -->
	...
	<main>
			...
			<div class="container">
				<div class="row">
					<div class="col-lg-8 posts-list">
						<div class="single-post">
							...
							<div class="blog_details">
								...
								<!-- 
								 ...
								 <a href="#"><i class="fa fa-heart j${DATA.mno}"></i> 찜하기</a>
						...
						<c:forEach var="p1list" items="${P1LIST}">
									<li><a href="https://watcha.com/"><img src="/project/assets/img/logo/watcha.png" id="watcha">${p1list.name}</a></li>
						</c:forEach>
						<c:forEach var="p2list" items="${P2LIST}">
									<li><a href="https://www.netflix.com/browse"><img src="/project/assets/img/logo/netflix.png">${p2list.name}</a></li>
						</c:forEach>
						<c:forEach var="p3list" items="${P3LIST}">
									<li><a href="https://movie.naver.com/"><img src="/project/assets/img/logo/youtubemovie.png">${p3list.name}</a></li>
						</c:forEach>
								</ul>
							</div>
						...
						<table class="table table-stripped" id="reviews">
							...
							<tbody>
								...
								<c:if test="${SID eq data.user_id}">
											<button type='button' class='btn btn-xs btn-success mdbtn' data-toggle='modal' data-target='#modifyModal' id="${data.one_no}">수정하기</button>
								</c:if>
										</td>
									</tr>
								</c:forEach>
						...
						<div class="col-lg-4">
							<div class="blog_right_sidebar">
								...
								<aside class="single_sidebar_widget popular_post_widget">
									...
									<div class="media post_item">
										<div class="media-body">
							<c:forEach var="ost" items="${OST}">
								<c:if test="${ost.platform_num == 100004}">
									<li><a href="https://watcha.com/"><img src="/project/assets/img/logo/melon.png" id="watcha">${ost.platform_num}</a></li>
											<a href="${ost.osturl}">
												<h3 style="color: #2d2d2d;">${ost.title}</h3>
											</a>
											<p>${ost.singer}</p>
								</c:if>
							</c:forEach>
							<c:forEach var="ost" items="${OST}">
								<c:if test="${ost.platform_num == 100005}">
									<li><a href="https://watcha.com/"><img src="/project/assets/img/logo/genie.jpg" id="genie">${ost.platform_num}</a></li>
											<a href="${ost.osturl}">
												<h3 style="color: #2d2d2d;">${ost.title}</h3>
											</a>
											<p>${ost.singer}</p>
							...
							<c:forEach var="ost" items="${OST}">
								<c:if test="${ost.platform_num == 100006}">
									<li><a href="https://watcha.com/"><img src="/project/assets/img/logo/youtubemovie.png" id="youtubemovie">${ost.platform_num}</a></li>
											<a href="${ost.osturl}">
												<h3 style="color: #2d2d2d;">${ost.title}</h3>
											</a>
											<p>${ost.singer}</p>
		...
		<form method="POST" action="/project/movie/reviewEditProc.proj" name="efrm" id="efrm"
										class="w3-col w3-round-large w3-card-4 w3-margin-bottom w3-padding">
								<input type="hidden" name="one_no" id="review_num" value="" >
								<div class="form-group">
									<label for="movie_no">영화 번호 </label>						
								<input class="form-control" id="movie_no" name="movie_num" value="${DATA.mno}" readonly>
								</div>
								<div class="form-group">
									<label for="review_text">리뷰 내용</label> 
									<input type="text" class="form-control" id="review_text" name="content"  value=""
										placeholder="댓글 내용을 입력해주세요">
								</div>
								<div class="form-group">
									<label for="review_writer">리뷰 작성자</label> 
									<input class="form-control" id="review_writer" name="user_id"  value="${SID}" readonly>
								</div>
		</form>
							</div>
